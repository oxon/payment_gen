...
describe DtaGen::DTA do
  ...
  it "should create a file" do
    DtaGen::DTA.create(dta_file_path) do |file|
  ...
  describe DtaGen::DTA, "file records" do
    before(:each) do
      ...
      @dta_file = DtaGen::DTA.create(dta_file_path) do |file|
        file << @record1
        file << @record2
      end
      @file_records = File.open(dta_file_path).readlines
    end
  ...
  describe DtaGen::DTA, "record sorting" do
    before(:each) do
      ...
      @dta_file = DtaGen::DTA.new(dta_file_path)
      @dta_file << @record1
      @dta_file << @record2
      @dta_file << @record3
      @dta_file << @record4
